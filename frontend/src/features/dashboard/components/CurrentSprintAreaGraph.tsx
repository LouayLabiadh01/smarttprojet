...
const CurrentSprintAreaGraph: React.FC<{
	...
	const handleTooltipChange = (data: any) => {
		// OR better, define the type more safely:
		// const handleTooltipChange = ({ activePayload }: TooltipProps<ValueType, NameType>) => {
				...
				<CardDescription className="flex justify-between">
					<div className="flex items-center gap-1 text-sm">
						<p className="flex items-center gap-1 rounded-full bg-accent px-2">
							...
							{format(
								props.currentSprint.start_date,
								"MMM d",
							)} - {format(props.currentSprint.end_date, "MMM d")}
						</p>
						<p className="ml-2 rounded-full bg-accent px-2">
							{differenceInDays(
								props.currentSprint.end_date,
								new Date(),
							)}{" "}
							days remaining
						</p>
