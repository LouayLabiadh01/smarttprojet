...
const UserComment = ({
	...
	console.log("this is commetns",comment)
	...
	async function handleDelete() {
		...
		if (comment.user.user_id !== user?.id) return;
		...
		>
			<div
				className={cn("flex items-end gap-2", {
					"flex-row-reverse": comment.user.user_id === user?.id,
			...
			>
				...
				<ContextMenu>
					<ContextMenuTrigger disabled={comment.user.user_id !== user?.id}>
						<div ref={commentRef}>
							<div
								className={cn(
							...
							{isLastInGroup && (
								<SimpleTooltip
									label={formatDateVerbose(
										new Date(comment.inserted_date.toString()),
									)}
								>
									<p
