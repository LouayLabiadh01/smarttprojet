...
export async function isAiLimitReached() {
	...
	const data = await kv.get(user.user_id + "aiDailyLimit");
	if (!data) {
		logger.debug("[AI] [Redis] Value not found, setting to 0");
		await kv.set(user.user_id + "aiDailyLimit", { count: 0 });
		return false;
	}
	...
	await kv.set(user.user_id + "aiDailyLimit", { count: parsedData.count + 1 });
	logger.debug(
