	...
	useEffect(() => {
		...
		if (lastViewed && lastViewed.length > 0) {
			const maxViewedAtDate = lastViewed.reduce((maxDate, task) => {
				return task.viewed_at > maxDate ? task.viewed_at : maxDate;
			}, lastViewed[0]?.viewed_at ?? new Date());

			// Only if there was a value to begin with will we actually accept the value
			if (lastViewed[0]?.viewed_at) {
	...
	const displayedHistory = useMemo(() => {
		if (lastViewedDate) {
			...
			let filteredHistory = sortedHistory.filter((item) =>
				isAfter(new Date(item.inserted_date), lastViewedDate),
			);
			...
			if (filteredHistory.length < 1 && sortedHistory.length > 0) {
				...
				filteredHistory = sortedHistory.filter((item) =>
					isAfter(new Date(item.inserted_date), oneHourAgo),
				);
			...
			<div className="flex flex-col gap-2 overflow-hidden">
				...
				{lastViewedDate && (
					<p className="text-sm text-muted-foreground">
						You last viewed at{" "}
						<SimpleTooltip
							label={formatDateVerbose(new Date(lastViewedDate))}
						>
							<span>{formatDateRelative(new Date(lastViewedDate))}</span>
						</SimpleTooltip>
					</p>
				)}
